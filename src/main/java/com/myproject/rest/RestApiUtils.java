...
/*import com.myproject.resource.CapabilityType;
import com.myproject.resource.FileUploadType;
import com.myproject.resource.GranteeCapabilitiesType;
import com.myproject.resource.GroupType;
import com.myproject.resource.ObjectFactory;
import com.myproject.resource.PermissionsType;
import com.myproject.resource.ProjectListType;
import com.myproject.resource.ProjectType;
import com.myproject.resource.SiteListType;
import com.myproject.resource.SiteType;
import com.myproject.resource.TableauCredentialsType;
import com.myproject.resource.TsRequest;
import com.myproject.resource.TsResponse;
import com.myproject.resource.WorkbookListType;
...
public class RestApiUtils {
    ...
    private static UriBuilder getApiUriBuilder() {
        return UriBuilder.fromPath(m_properties.getProperty("server.host") + "/api/2.0");
    }
    /**
    ...
    private static void initialize() {
        try {
            m_properties.load(new FileInputStream("/Users/anand/Documents/RESTfulIMSService/src/main/resources/config.properties"));
        	...
        	throw new IllegalStateException("Failed to create SAXException API");
        } catch (JAXBException ex) {
            throw new IllegalStateException("Failed to create Jaxbcontext");
    ...
    public ViewListType queryViewsForWorkbooks(TableauCredentialsType credential, String siteId, String workbookId) {
...
/*    public ViewListType queryWorkbookPreviewImage(TableauCredentialsType credential, String siteId, String workbookId) {

        m_logger.info(String.format("Querying WorkbookPreviewImage for workbook '%s'.", workbookId));

        String url = Operation.QUERY_WORKBOOK_PREVIEWIMAGE.getUrl(siteId, workbookId);
        ...
        // Verifies that the response has a workbooks element
        if (response.getViews() != null) {
            m_logger.info("Query WorkbookPreviewImag for workbook is successful!");
    ...
    public String queryWorkbookPreviewImage(TableauCredentialsType credential, String siteId, String workbookId) {
       ...
       // TsResponse response = get(url, credential.getToken());
        ...
        m_logger.info("Response for WorkbookPreviewImag: \n" + responseXML);
